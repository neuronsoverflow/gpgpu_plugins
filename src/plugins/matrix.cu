 ...
 ******************************************************************************/
template <int BLOCK_SIZE> __global__ void matrixMulCUDA(const Matrix A, const Matrix B, Matrix C)
{
    float cValue = 0;
    ...
    if (row < C.height && col < C.width)
        C.elements[((blockIdx.y * blockDim.y + threadIdx.y) * C.width) + (blockIdx.x * blockDim.x) + threadIdx.x] =
            cValue;
}
...
//////// debug
    ...
    cudaError_t error = cudaMemcpy(C->elements, d_C.elements, mem_size_C, cudaMemcpyDeviceToHost);
    if (error != cudaSuccess)
    {
