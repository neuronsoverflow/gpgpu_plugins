...
enum inputs { MODE, INPUT, OUTPUT, SOURCE, DISPLAY };
...
int main(int argc, char** argv)
{
   if (argc == 1)
   {
      std::cout << "Usage: " << argv[0]
                << " searchMode inputFile outputFile sourceNode\n";
      return 0;
   }
...
int runBFS()
{
   ...
   {
      std::cout << "Source node out of bounds. Must be a vertex index 0-"
                << graph.vertexCount - 1 << ".\n";
      return ERROR;
   }
...
int writeGraphSearchResults(const GraphData& graph, int* costs, int source)
{
   ...
   for (int i = 0; i < graph.vertexCount; i++)
   {
      fout << std::setw(5) << std::left << i << std::right << " => "
           << costs[i] << '\n';
   }
   fout.close();
   return OK;
